...
public class SqlDependencyListener<T> : IDisposable
{
    ...
    public string QueueName
        => _options.QueueName == null || string.IsNullOrWhiteSpace(_options.QueueName) ? $"LISTENER_QUEUE_{Identity.ToString()}" : _options.QueueName;
    public string ServiceName
        => _options.ServiceName == null || string.IsNullOrWhiteSpace(_options.ServiceName) ? $"LISTENER_SERVICE_{Identity.ToString()}" : _options.ServiceName;
    public string TriggerName
        => _options.TriggerName == null || string.IsNullOrWhiteSpace(_options.TriggerName) ? $"TR_LISTENER_{Identity.ToString()}" : _options.TriggerName;
    public string InstallProcedureName
        => _options.InstallProcedureName == null || string.IsNullOrWhiteSpace(_options.InstallProcedureName) ? $"SP_INSTALL_LISTENER_{Identity.ToString()}" : _options.InstallProcedureName;
    public string UninstallProcedureName
        => _options.UninstallProcedureName == null || string.IsNullOrWhiteSpace(_options.UninstallProcedureName) ? $"SP_UNINSTALL_LISTENER_{Identity.ToString()}" : _options.UninstallProcedureName;
